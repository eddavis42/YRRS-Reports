...
params:
  ...
  cnty: "c_santa_fe"
  regions: 2
  region: "Northeast Public Health Region"
  year: "2021"
...
year1 <- year1 %>% mutate(
  cntytxt = values2labels(cnty)
)

yrrs_cnty_1 <- svydesign(id=~psu, strata=~strata, weights=~fwt_cnty, data=year1)
...
#Reconfigure study designs after making changes to variable names
...
The NM Department of Health receives support for the NM YRRS from the Centers for Disease Control and Prevention in cooperation through Grant number 6NU87PS0043 14-05-01.
...
yrbss_ms_cnty <- subset(yrbss_NM, cntytxt=={params$counties})
#Response rates
...
Totals <- c(length(yrbss_ms_cnty$cnty)
            #Girls/boys
            , length(na.omit(yrbss_ms_cnty$sex[(yrbss_ms_cnty$sex==1)]))
            , length(na.omit(yrbss_ms_cnty$sex[(yrbss_ms_cnty$sex==2)]))
            #Race/ethnicity
            , length(na.omit(yrbss_ms_cnty$ai[(yrbss_ms_cnty$ai==1)]))
            , length(na.omit(yrbss_ms_cnty$as[(yrbss_ms_cnty$as==1)]))
            , length(na.omit(yrbss_ms_cnty$aa[(yrbss_ms_cnty$aa==1)]))
            , length(na.omit(yrbss_ms_cnty$hisp[(yrbss_ms_cnty$hisp==1)]))
            , length(na.omit(yrbss_ms_cnty$pi[(yrbss_ms_cnty$pi==1)]))
            , length(na.omit(yrbss_ms_cnty$wh[(yrbss_ms_cnty$wh==1)]))
            #Grade level
            , length(na.omit(yrbss_ms_cnty$grade[(yrbss_ms_cnty$grade==1)]))
            , length(na.omit(yrbss_ms_cnty$grade[(yrbss_ms_cnty$grade==2)]))
            , length(na.omit(yrbss_ms_cnty$grade[(yrbss_ms_cnty$grade==3)]))
            #, length(na.omit(yrbss_ms_cnty$grade[(yrbss_ms_cnty$grade==4)]))
            , length(yrbss_ms_cnty$grade[is_tagged_na(yrbss_ms_cnty$grade, "a")]))
...
The response rate for `r {params$counties}` County was `r signif(yrbss_NM$rr_cnty[(yrbss_NM$cntytxt=={params$counties})][1], 3)*100`%.  
...
Table_function <- function(varlist) {
  ...
  colnames(Data_frame_values) <- c("Indicator", "cnty_Value", "cnty_ci_l", "cnty_ci_u", "cnty_ci", "NM_Value", "NM_ci_l", "NM_ci_u", "NM_ci")
  ########################################################
  ...
  for(i in 1:length(varlist)){
    ...
    #County level values
    Data_frame_values[i,2:4] <- round((svyby(as.formula( paste0("~", varname)), ~cntytxt=={params$counties}, yrrs_cnty, svyciprop, vartype ="ci", df = Design_df, na.rm=T))[2,2:4] *100, 1)
    Data_frame_values[i,5] <- paste0("(",Data_frame_values[i,3], "-",Data_frame_values[i,4],")")
    
    #NM level values
    Data_frame_values[i,7:8] <- round((confint(svymean(yrbss_NM[varname], yrrs_nm, na.rm=T)))[1:2] *100, 1)
    Data_frame_values[i,6] <- round((svymean(yrbss_NM[varname], yrrs_nm, na.rm=T))[1] *100, 1)
...
barplot_county(c("helmet", "seatbelt","rodedui","weapon", "fight", "bully","cbully"), "Personal Safety",ms=TRUE )
```
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height = 4.5, fig.width=6.5}
barplot_county(c("mental", "consider","plan","suiatt"), "Mental Health and Suicidal Behaviors", ms=TRUE)
```
...
print(barplot_county(c("cigevr","cig30","cigar","spittob", "hookah","vapeevr", "vape30","envsmoke"), "Tobacco Use", ms=TRUE))
```
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height = 5, fig.width=6.5}
print(barplot_county(c("drnkevr","drnk30","drnk11","drnk5"), "Alcohol Use", ms=TRUE))
```
...
print(barplot_county(c("marievr", "mari30", "mari11", "synevr", "cocaevr", "inhevr", "rxpainevr", "rxpain"), "Drug Use", ms=TRUE))
```
...
print(barplot_county(c("sexevr","sex11"), "Sexual Behavior", ms=TRUE))
```
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height = 4, fig.width=6.5}
print(barplot_county(c("qnbk7day","descowt","losewt", "fast", "purge", "dietpill"), "Body Weight and Nutrition", ms=TRUE))
print(barplot_county(c("qnpa0day", "qnpa5day","qnpa7day","screent3h", "qndlype", "team"), "Physical Activity", ms=TRUE))
```
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height = 6, fig.width=6.5}
print(barplot_county(c("concussion","dental","sleep8", "gamble", "homeless"), "Other Characteristics", ms=TRUE))
```
\newpage
...
### Personal Safety
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height = 4.5, fig.width=6.5}
trendplot(c("helmet", "seatbelt"), graph_subtitle = "Personal Safety: Behaviors Associated with Unintentional Injury", ms=TRUE)

```
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height = 17, fig.width=12, fig.align = "center"}
barplot_group(c("helmet", "seatbelt"), "Personal Safety: Behaviors Associated with Unintentional Injury", ms=TRUE)
```

\newpage

```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height = 4.5, fig.width=6.5}
trendplot(c("weapon", "fight"), graph_subtitle = "Personal Safety: Behaviors Associated with Violence", ms=TRUE)
```
\newpage
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height = 17, fig.width=12, fig.align = "center"}
barplot_group(c("weapon", "fight"), "Personal Safety: Behaviors Associated with Violence", ms=TRUE)
```
\newpage
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height = 4.5, fig.width=6.5}
trendplot(c("bully", "cbully"), graph_subtitle = "Personal Safety: Bullying", ms=TRUE)
```
\newpage
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height = 17, fig.width=12, fig.align = "center"}
barplot_group(c("bully", "cbully"), "Personal Safety: Bullying", ms=TRUE)
```

\newpage
### Mental Health and Suicidal Behaviors   
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height = 4.5, fig.width=6.5}
trendplot(c("consider", "plan", "suiatt"), graph_subtitle = "Mental Health and Suicidal Behaviors", ms=TRUE)
```
\newpage
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height = 17, fig.width=12, fig.align = "center"}
barplot_group(c("mental", "consider", "plan", "suiatt"), "Mental Health and Suicidal Behaviors", ms=TRUE)
```
...
options(survey.adjust.domain.lonely=TRUE)
options(survey.lonely.psu="adjust")
trendplot(c("vapeevr", "cigevr"), graph_subtitle = "Tobacco Use and Exposure: Ever used Tobacco", ms=TRUE)
trendplot(c("vape30", "cig30", "cigar"), graph_subtitle = "Tobacco Use and Exposure: Current Tobacco use", ms=TRUE)
```

```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height = 4.5, fig.width=6.5}
trendplot(c("qntob5", "hookah", "spittob"), graph_subtitle = "Tobacco Use and Exposure: Current Tobacco use", ms=TRUE)
```
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height = 17, fig.width=12, fig.align = "center"}
barplot_group(c("cigevr", "cig30", "cig111"), "Tobacco Use and Exposure", ms=TRUE)
barplot_group(c("vapeevr", "vape30", "cigar", "hookah","spittob"), "Tobacco Use and Exposure", ms=TRUE)
barplot_group(c("qntob5", "notob5", "envsmoke"), "Tobacco Use and Exposure", ms=TRUE)
```

\newpage
### Alcohol Use   
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height = 6, fig.width=7, fig.align = "center"}
trendplot(c("drnkevr", "drnk30", "drnk5", "drnk11"), graph_subtitle = "Alcohol Use", ms=TRUE)
```
\newpage
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height = 17, fig.width=12, fig.align = "center"}
barplot_group(c("drnkevr", "drnk30", "drnk11"), "Alcohol Use", ms=TRUE)
barplot_group(c("drnk5", "rodedui"), "Alcohol Use", ms=TRUE)
barplot_group(c("alceasy", "alcwrng", "alcharm", "alcparwg"), "Alcohol Use", ms=TRUE)
```
...
### Drug Use   
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height = 6, fig.width=7, fig.align = "center"}
trendplot(c("marievr", "mari30", "mari11"), graph_subtitle = "Drug Use: Marijuana", ms=TRUE)
trendplot(c("synevr", "cocaevr", "inhevr", "rxpainevr", "rxpain"), graph_subtitle = "Drug Use: Other Drugs", ms=TRUE)
```
\newpage
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height = 17, fig.width=12, fig.align = "center"}
barplot_group(c("marievr", "mari30", "mari11"), "Drug Use: Marijuana", ms=TRUE)
barplot_group(c("synevr", "cocaevr", "inhevr", "rxpainevr", "rxpain"), "Drug Use: Other Drugs", ms=TRUE)
```

\newpage
### Sexual Behaviors   
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height = 6, fig.width=7, fig.align = "center"}
trendplot(c("sexevr", "condom"), graph_subtitle = "Sexual Behaviors", ms=TRUE)
```

\newpage

```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height = 17, fig.width=12, fig.align = "center"}
barplot_group(c("sexevr", "sex11", "sex3", "condom","nocondom"), "Sexual behaviors", ms=TRUE)
```

### Body Weight and Nutrition   
```{r echo=FALSE, fig.align="center", fig.height=6, fig.width=7.5, message=FALSE, warning=FALSE}
trendplot(c("qnbk7day", "fast"), graph_subtitle = "Body Weight and Nutrition", ms=TRUE)
```
\newpage
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height = 17, fig.width=12, fig.align = "center"}
barplot_group(c("qnbk7day", "descowt", "losewt", "fast", "purge", "dietpill"), "Body Weight and Nutrition", ms=TRUE)
```

\newpage
### Physical Activity   
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height = 6, fig.width=7, fig.align = "center"}
trendplot(c("qnpa7day", "qndlype"), graph_subtitle = "Physical Activity", ms=TRUE)
```

\newpage

```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height = 17, fig.width=12, fig.align = "center"}
barplot_group(c("qnpa0day", "qnpa5day", "qnpa7day"), "Physical Activity", ms=TRUE)
barplot_group(c("screent3h", "qndlype", "team"), "Physical Activity", ms=TRUE)
```

\newpage
### Other Characteristics   
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height = 6, fig.width=7, fig.align = "center"}
trendplot(c("concussion", "dental", "sleep8", "gamble"), graph_subtitle = "Other Characteristics", ms=TRUE)
```
\newpage   
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height = 17, fig.width=12, fig.align = "center"}
barplot_group(c("concussion", "dental", "sleep8", "gamble", "homeless"), "Other Characteristics", ms=TRUE)
```
...
barplot_county_2(c("qn70", "qn71"), "Resiliency/Protective Factors in the Home", ms=TRUE) + guides(x.sec = guide_none(title = "It is \"Very much true\" or \"Pretty much true\" that in my home..."))


barplot_county_2(c("qn72", "qn73"), "Resiliency/Protective Factors in the School", ms=TRUE) + guides(x.sec = guide_none(title = "It is \"Very much true\" or \"Pretty much true\" that in my school..."))

```

\newpage
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height = 4, fig.width=6, fig.align = "center"}
barplot_county_2(c("qn76", "qn77"), "Resiliency/Protective Factors in the Community", ms=TRUE)  + guides(x.sec = guide_none(title = "It is \"Very much true\" or \"Pretty much true\" that outside my home and school..."))

barplot_county_2(c("qn74", "qn75"), "Resiliency/Protective Factors with Peers and Personal", ms=TRUE) + guides(x.sec = guide_none(title = "It is \"Very much true\" or \"Pretty much true\" that..."))
```
...
barplot_resiliency("fight", c("r377", "r376", "r375", "r374", "r373", "r372", "r371", "r370"), "county", ms=TRUE)
...
barplot_resiliency("weapon", c("r377", "r376", "r375", "r374", "r373", "r372", "r371", "r370"), "county", ms=TRUE)
```
#### How to read this chart: {.unlisted .unnumbered}
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height = 13, fig.width=12, fig.align = "center", cache.rebuild=TRUE}
barplot_resiliency("consider", c("r377", "r376", "r375", "r374", "r373", "r372", "r371", "r370"), "county", ms=TRUE)
```
#### How to read this chart: {.unlisted .unnumbered}
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height = 13, fig.width=12, fig.align = "center", cache.rebuild=TRUE}
barplot_resiliency("suiatt", c("r377", "r376", "r375", "r374", "r373", "r372", "r371", "r370"), "county", ms=TRUE)
```
#### How to read this chart: {.unlisted .unnumbered}
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height = 13, fig.width=12, fig.align = "center", cache.rebuild=TRUE}
barplot_resiliency("cig30", c("r377", "r376", "r375", "r374", "r373", "r372", "r371", "r370"), "county", ms=TRUE)
```
#### How to read this chart: {.unlisted .unnumbered}
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height = 13, fig.width=12, fig.align = "center", cache.rebuild=TRUE}
barplot_resiliency("vape30", c("r377", "r376", "r375", "r374", "r373", "r372", "r371", "r370"), "county", ms=TRUE)
```
#### How to read this chart: {.unlisted .unnumbered}
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height = 13, fig.width=12, fig.align = "center", cache.rebuild=TRUE}
barplot_resiliency("drnkevr", c("r377", "r376", "r375", "r374", "r373", "r372", "r371", "r370"), "county", ms=TRUE)
```
#### How to read this chart: {.unlisted .unnumbered}
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height = 13, fig.width=12, fig.align = "center", cache.rebuild=TRUE}
barplot_resiliency("drnk30", c("r377", "r376", "r375", "r374", "r373", "r372", "r371", "r370"), "county", ms=TRUE)
```
#### How to read this chart: {.unlisted .unnumbered}
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height = 13, fig.width=12, fig.align = "center", cache.rebuild=TRUE}
barplot_resiliency("marievr", c("r377", "r376", "r375", "r374", "r373", "r372", "r371", "r370"), "county", ms=TRUE)
```
#### How to read this chart: {.unlisted .unnumbered}
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height = 13, fig.width=12, fig.align = "center", cache.rebuild=TRUE}
barplot_resiliency("sexevr", c("r377", "r376", "r375", "r374", "r373", "r372", "r371", "r370"), "county", ms=TRUE)
```
#### How to read this chart: {.unlisted .unnumbered}
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height = 13, fig.width=12, fig.align = "center", cache.rebuild=TRUE}
barplot_resiliency("qnpa7day", c("r377", "r376", "r375", "r374", "r373", "r372", "r371", "r370"), "county", ms=TRUE)
```
#### How to read this chart: {.unlisted .unnumbered}
...
```{r echo=FALSE, warning=FALSE, message=FALSE}
#yrbss_ms_cnty <- subset(yrrs2019, cntytxt=={params$counties})
library(readxl)
...
#sum(yrbss_ms_cnty$v1[yrbss_ms_cnty$v1==2], na.rm=TRUE) #Get number of responses with answer


Totals <- c(#How old are you?
            length(na.omit(yrbss_ms_cnty$v1[yrbss_ms_cnty$v1==1])),
            length(na.omit(yrbss_ms_cnty$v1[yrbss_ms_cnty$v1==2])),
            length(na.omit(yrbss_ms_cnty$v1[yrbss_ms_cnty$v1==3])),
            length(na.omit(yrbss_ms_cnty$v1[yrbss_ms_cnty$v1==4])),
            length(na.omit(yrbss_ms_cnty$v1[yrbss_ms_cnty$v1==5])),
            length(na.omit(yrbss_ms_cnty$v1[yrbss_ms_cnty$v1==6])),
            length(na.omit(yrbss_ms_cnty$v1[yrbss_ms_cnty$v1==7])),
            #What is your sex?
            length(na.omit(yrbss_ms_cnty$v2[yrbss_ms_cnty$v2==1], na.rm=TRUE)),
            length(na.omit(yrbss_ms_cnty$v2[yrbss_ms_cnty$v2==2], na.rm=TRUE)),
            ...
            length(na.omit(yrbss_ms_cnty$v1[yrbss_ms_cnty$v4==1], na.rm=TRUE)),
            length(na.omit(yrbss_ms_cnty$v1[yrbss_ms_cnty$v4==2], na.rm=TRUE)),
            #What is your race?
            length(na.omit(yrbss_ms_cnty$ai[(yrbss_ms_cnty$ai==1)], na.rm=TRUE)),
            length(na.omit(yrbss_ms_cnty$as[yrbss_ms_cnty$as==1], na.rm=TRUE)),
            length(na.omit(yrbss_ms_cnty$aa[yrbss_ms_cnty$aa==1], na.rm=TRUE)),
            length(na.omit(yrbss_ms_cnty$pi[yrbss_ms_cnty$pi==1], na.rm=TRUE)),
            length(na.omit(yrbss_ms_cnty$wh[yrbss_ms_cnty$wh==1], na.rm=TRUE)))
...
Percents <- c(
            ...
            svyby(~v4==2, ~cntytxt=={params$counties},yrrs_cnty, svyciprop, vartype ="ci", df = Design_df, na.rm=T)[2,2]%>% scales::percent(accuracy=0.1),
            #What is your race?
...
for (i in 28:440){
  ...
  Questionnaire[i, 9] <- round((svyby(as.formula( paste0("~(", qnvar, "==", Response,")")), ~cntytxt=={params$counties}, yrrs_cnty, svyciprop, vartype ="ci", df = Design_df, na.rm=T))[2,2] *100, 1)
  
  ci_l <- round((svyby(as.formula( paste0("~(", qnvar, "==", Response,")")), ~cntytxt=={params$counties}, yrrs_cnty, svyciprop, vartype ="ci", df = Design_df, na.rm=T))[2,3] *100, 1)
  ci_u <- round((svyby(as.formula( paste0("~(", qnvar, "==", Response,")")), ~cntytxt=={params$counties}, yrrs_cnty, svyciprop, vartype ="ci", df = Design_df, na.rm=T))[2,4] *100, 1)
